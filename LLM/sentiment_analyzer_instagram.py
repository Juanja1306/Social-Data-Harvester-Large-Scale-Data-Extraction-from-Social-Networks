...
# Cliente asíncrono de OpenAI (Oficial)
client = AsyncOpenAI(
    api_key=OPENAI_API_KEY
    # No necesitamos base_url para OpenAI oficial
)

MODELO = "gpt-4o-mini" # Modelo rápido y económico de OpenAI
...
async def analizar_sentimiento_deepseek(texto: str, tipo: str = "contenido") -> Dict:
    ...
    try:
        # Prompt optimizado para análisis de sentimiento en contexto de Instagram
        prompt = f"""Analiza el sentimiento del siguiente texto de Instagram y proporciona una clasificación clara.
...
async def procesar_instagram_concurrente(csv_file: str = "resultados.csv") -> List[Dict]:
    """
    Procesa todas las publicaciones de Instagram de forma concurrente.
    Args:
    ...
    df_instagram = df[df['RedSocial'] == 'Instagram'].copy()
    ...
    for _, row in df_instagram.iterrows():
        data_parsed = parse_instagram_data(row['Data'])
...
def generar_reporte(resultados: List[Dict]) -> str:
    ...
    reporte = f"""
        {'='*70}
        REPORTE DE ANÁLISIS DE SENTIMIENTOS - INSTAGRAM (OpenAI)
        {'='*70}
...
def start_instagram_analysis(csv_file: str = "resultados.csv", save_to_file: bool = True):
    """
    ...
    try:
        ...
        if not resultados:
            return ("No se procesaron publicaciones. Verifica que existan datos de Instagram en el CSV.", []) if not save_to_file else "No se procesaron publicaciones. Verifica que existan datos de Instagram en el CSV."
        ...
        if save_to_file:
            try:
                ...
                print(f"[Instagram] Resultados guardados en {ARCHIVO_RESULTADOS_JSON}")
            except Exception as e:
                print(f"[Instagram] Error guardando JSON: {e}")
        ...
        if save_to_file:
            try:
                with open(ARCHIVO_REPORTE, 'w', encoding='utf-8') as f:
                    f.write(reporte)
                print(f"[Instagram] Reporte guardado en {ARCHIVO_REPORTE}")
            except Exception as e:
                print(f"[Instagram] Error guardando reporte: {e}")
        ...
        error_msg = f"Error crítico en análisis de Instagram: {str(e)}"
